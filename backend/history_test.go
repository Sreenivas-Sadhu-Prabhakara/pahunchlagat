...
func TestPostgresStore_RoundTrip(t *testing.T) {
	...
	in, _ := json.Marshal(Input{GoodsValue: 10000, GSTPct: 18, Units: 100})
	saved, err := store.Save(Record{Input: in, Headline: 134.7, Label: "composition"})
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if saved.ID == 0 {
	...
	if len(items) == 0 || items[0].ID != saved.ID || items[0].Label != "composition" {
